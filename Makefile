# Path to the Arrow C++ source and build directory
#SCRATCH=/pscratch/sd/l/lbl59
PARQUET_BUILD := arrow-libs/include
PARQUET_LIBS := arrow-libs/lib64
RPATH_FLAG := -Wl,-rpath=$(PARQUET_LIBS)
...
TARGET=SedentoValleySimulation
EXECUTABLE=$(TARGET)
...
borg: CC=mpicxx
borg: LIBS += -lborgmm  #-lborgms 
borg: CFLAGS += -DPARALLEL -fopenmp -march=native -O2 -I $(PARQUET_BUILD)
borg: LDFLAGS += -L $(PARQUET_LIBS) $(RPATH_FLAG)
borg: LDLIBS+=-lparquet -larrow -lsnappy -lzstd -lz -pthread
borg: all

gcc: CC=g++
gcc: CFLAGS+=-O2 -march=native -fopenmp -I $(PARQUET_BUILD)
gcc: LDFLAGS+=-L $(PARQUET_LIBS) $(RPATH_FLAG)
gcc: LDLIBS+=-lparquet -larrow -lsnappy -lzstd -lz -pthread
gcc: all
...
$(TARGET): $(OBJECTS)
	$(CC) -I. $(LDFLAGS) $(OBJECTS) $(CFLAGS) -o $@ -L$(LIB_DIR) $(LIBS) $(DEFINES) $(LDLIBS)
