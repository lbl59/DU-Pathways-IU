...
def render_home() -> None:
    ...
    st.write(
        ...
        "accompanying paper. Use the sidebar, or the gallery below, to open a "
        "figure explorer."
    )
    ...
    with st.expander("About this repository"):
        st.write(
            ...
            "figures can be added by dropping a new `N_Figure_Name.py` file "
            "into `pages/` — along with a matching "
            "`figures/results_figN_*.jpg` thumbnail — and it will "
            "automatically appear both in the sidebar navigation and in the "
            "gallery above."
        )


# ---------------------------------------------------------------------------
# Navigation: an explicit st.Page per screen lets us set the sidebar label
# for the landing page ("Home Page") independently of this file's name,
# while every figure-explorer page is still auto-discovered from pages/.
# ---------------------------------------------------------------------------
...
nav = st.navigation([home_page, *figure_pages])
nav.run()
