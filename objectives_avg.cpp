...
double ObjectivesCalculator::calculateReliabilityObjective(
    ...
    if (check_non_zero > 0) {
        double obj_value = 1. - (double)accumulate(year_reliabilities.begin(),
                                       year_reliabilities.end(),
                                       0.0) / (n_years * n_realizations);
...
double ObjectivesCalculator::calculateRestrictionFrequencyObjective(
    ...
    if (!restriction_data.empty()) {
        ...
        double restriction_frequency = 0;  // ACTIVATE FOR AVERAGE CASE
        ...
        double obj_value = restriction_frequency / (n_realizations * n_years);   // ACTIVATE FOR AVERAGE CASE
        if (std::isinf(obj_value)) {
...
double ObjectivesCalculator::calculateNetPresentCostInfrastructureObjective(
    ...
    double infrastructure_npc = 0;  // ACTIVATE FOR AVERAGE CASE
    ...
    return infrastructure_npc / n_realizations;  // ACTIVATE FOR AVERAGE CASE
}  
...
double ObjectivesCalculator::calculateWorseCaseCostsObjective(
    ...
    // Calculate average
    double obj_value = accumulate(worse_year_financial_costs.begin(),
