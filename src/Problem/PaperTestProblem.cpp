...
void PaperTestProblem::setProblemDefinition(BORG_Problem &problem)
{
    ...
    BORG_Problem_set_epsilon(problem, 2, 5.0);
...
/**
 * Runs the Sedento Valley test case for demonstration paper
 * @param vars
...
int PaperTestProblem::functionEvaluation(double *vars, double *objs, double *consts) {
    ...
    /**
     * UPDATE ON 2025-08-24 BY Lillian Lau.
     * Updating added_storage_college_rock_expansion_low from 100 to 500 
     * and added_storage_college_rock_expansion_high 2500 to 1000
     * to match descriptions in paper
     */
    ...
    // ==================== SET UP RDM FACTORS ============================
    // CHANGED BY LILLIAN LAU ON 2025-05-14
    // modified to check for empty vectors individually
    ...
    if (water_sources_rdm.empty()) {
        water_sources_rdm = std::vector<vector<double>>(n_realizations, vector<double>(51, 1.));
    }
    if (policies_rdm.empty()) {
        policies_rdm = std::vector<vector<double>>(n_realizations, vector<double>(4, 1.));
    }
    if (actions_rdm.empty()) {
        // ADDED ON 2025-04-28 by Lillian Lau.
        actions_rdm = std::vector<vector<double>>(n_realizations, vector<double>(11, 0.0));
    }
...
//Beginning with Reservoir continuity
    ...
    /**
     * UPDATE ON 2025-08-24 BY Lillian Lau.
     * Reduced New River Reservoir storage from 7300 to 3700 to match paper
     */
    vector<double> new_river_res_storage = {0, 3700 * calibrate_volume_multiplier};
    ...
    /**
     * UPDATE ON 2025-08-24 BY Lillian Lau.
     * Reduced Sugar Creek storage from 4500 to 2909 to match paper
     */
    ...
    /**
     * UPDATE ON 2025-08-24 BY Lillian Lau.
     * Decreasrf Granite Quarry storage from 1500 to 200 to match paper
     */
    vector<double> granite_quarry_storage = {0, 1500}; //200};
...
// Create existing reservoirs
    ...
    //FIXME check bond, this one is from little river raleigh
 ...
 */
    ...
    // CHANGED BY LILLIAN LAU ON 2025-04-28 to include actions_rdm_multiplier ID numbers
    //FIXME: bond etc need to be updated, should chat about demand buffer
    Utility watertown((char *) "Watertown", 0, 8, demand_watertown, demand_n_weeks, watertown_annual_payment,
    ...
    //FIXME: bond etc need to be updated, should chat about demand buffer
    Utility dryville((char *) "Dryville", 1, 9, demand_dryville, demand_n_weeks, dryville_annual_payment,
    ...
    Utility fallsland((char *) "Fallsland", 2, 10, demand_fallsland, demand_n_weeks, fallsland_annual_payment,
    ...
    // CHANGED BY LILLIAN LAU ON 2025-04-28 to include actions_rdm_multiplier ID number                                                      
    Restrictions restrictions_w(0, 0,
                                restriction_stage_multipliers_watertown,
                                restriction_stage_triggers_watertown);

    Restrictions restrictions_d(1, 1,
                                restriction_stage_multipliers_dryville,
                                restriction_stage_triggers_dryville);

    Restrictions restrictions_f(2, 2,
    ...
    vector<int> actions_rdm_multiplier_ids = {3, 4}; // ADDED BY LILLIAN LAU ON 2025-04-29 to include actions_rdm_multiplier ID numbers
    ...
    Transfers t(3, 0, 1, 35,
                buyers_ids,
                actions_rdm_multiplier_ids,   // CHANGED BY LILLIAN LAU ON 2025-04-29 to include actions_rdm_multiplier ID numbers
    ...
    if (import_export_rof_tables == EXPORT_ROF_TABLES) {
        s = new Simulation(water_sources,
                           ...
                           water_sources_rdm,
                           policies_rdm,
                           actions_rdm,  // ADDED ON 2025-04-28 by Lillian Lau
                           n_weeks,
                           realizations_to_run,
    ...
    objectives = calculateAndPrintObjectives(false);
        ...
        int i = 0;
        objs[i] = 1. - min(min(objectives[i], objectives[NUM_OBJECTIVES + i]),
        		   objectives[2*NUM_OBJECTIVES + i]);
        for (i = 1; i < 5; ++i) {
            objs[i] = max(max(objectives[i], objectives[NUM_OBJECTIVES + i]),
      	                  objectives[2*NUM_OBJECTIVES + i]);
        }
...
void PaperTestProblem::readInputData(int mode) {
    cout << "Reading input data." << endl;
    string data_dir = DEFAULT_DATA_DIR + BAR;

    string rdm_tseries_dir;
    if (rdm_no != NON_INITIALIZED) {
        data_dir = rdm_directory + BAR + "rdm_" + to_string(rdm_no) + BAR;
    }

#pragma omp parallel num_threads(omp_get_thread_num())
    {
        
    if (mode == 0) {  // if reading in CSV files
        // READING INFLOW DATA
#pragma omp single
        streamflows_durham = Utils::parse2DCsvFile(
                io_directory + data_dir + 
                "inflows" + inflow_demand_evap_suffix +
...
#pragma omp single
        streamflows_crabtree = Utils::parse2DCsvFile(
                io_directory + data_dir + 
                "inflows" + inflow_demand_evap_suffix +
                BAR + "crabtree_creek_inflows.csv", n_realizations);
#pragma omp single
...
#pragma omp single
        evap_durham = Utils::parse2DCsvFile(
                io_directory + data_dir + 
                "evaporation" + inflow_demand_evap_suffix +
...
#pragma omp single
        ...
        demand_watertown = Utils::parse2DCsvFile(
                 io_directory + data_dir + 
                 "demands" + inflow_demand_evap_suffix +
                BAR + "cary_demand.csv", n_realizations);
#pragma omp single
        // Dryville is Durham
        demand_dryville = Utils::parse2DCsvFile(
                io_directory + data_dir + "demands" + inflow_demand_evap_suffix +
                BAR + "durham_demand.csv", n_realizations);
#pragma omp single
        // Fallsland is Raleigh
        demand_fallsland = Utils::parse2DCsvFile(
                io_directory + data_dir + "demands" + inflow_demand_evap_suffix +
                BAR + "raleigh_demand.csv", n_realizations);

    } else if (mode == 1) { // if reading in Parquet files
#pragma omp single
        streamflows_durham = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "durham_inflows.parquet", n_realizations); 
#pragma omp single
        streamflows_flat = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "falls_lake_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_swift = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "lake_wb_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_llr = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "little_river_raleigh_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_phils = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "stone_quarry_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_cane = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "cane_creek_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_morgan = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "university_lake_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_crabtree = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "crabtree_creek_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_haw = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "jordan_lake_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_lillington = Utils::parse2DParquetFile(
                io_directory + data_dir + 
                "inflows" + inflow_demand_evap_suffix + 
                BAR + "lillington_inflows.parquet", n_realizations);

#pragma omp single
        streamflows_clayton = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "inflows" + inflow_demand_evap_suffix +
                BAR + "clayton_inflows.parquet", n_realizations);

#pragma omp single
        evap_durham = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "durham_evap.parquet", n_realizations);

#pragma omp single
        evap_falls_lake = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "falls_lake_evap.parquet", n_realizations);

#pragma omp single
        evap_jordan_lake = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "jordan_lake_evap.parquet", n_realizations);

#pragma omp single
        evap_little_river = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "little_river_raleigh_evap.parquet", n_realizations);

#pragma omp single
        evap_wheeler_benson = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "lake_wb_evap.parquet", n_realizations);

#pragma omp single
        evap_owasa = Utils::parse2DParquetFile(
                io_directory + data_dir +
                "evaporation" + inflow_demand_evap_suffix +
                BAR + "jordan_lake_evap.parquet", n_realizations);
...
#pragma omp single
        // Watertown is Cary
        demand_watertown = Utils::parse2DParquetFile(
                io_directory + data_dir + "demands" +
                inflow_demand_evap_suffix +
                BAR + "cary_demands.parquet", n_realizations);

#pragma omp single
        // Dryville is Durham
        demand_dryville = Utils::parse2DParquetFile(
                io_directory + data_dir + "demands" +
                inflow_demand_evap_suffix +
                BAR + "durham_demands.parquet", n_realizations);

#pragma omp single
        // Fallsland is Raleigh
        demand_fallsland = Utils::parse2DParquetFile(
                io_directory + data_dir + "demands" +
                inflow_demand_evap_suffix +
                BAR + "raleigh_demands.parquet", n_realizations);
    }
