 ...
 */
Restrictions::Restrictions(const int id, const int action_multiplier_id, // ADDED ON 2025-04-28 by Lillian Lau.
        ...
        : DroughtMitigationPolicy(id, RESTRICTIONS),
          ...
          action_multiplier_id(action_multiplier_id) {  // ADDED ON 2025-04-28 by Lillian Lau.
    utilities_ids = vector<int>(1, id);
}

Restrictions::Restrictions(
        const int id, const int action_multiplier_id,  // ADDED ON 2025-04-28 by Lillian Lau.
        const vector<double> &stage_multipliers,
        const vector<double> &stage_triggers,
...
Restrictions::Restrictions(const Restrictions &restrictions) :
        ...
        restricted_weekly_average_volumetric_price(restrictions.restricted_weekly_average_volumetric_price), 
        action_multiplier_id(restrictions.action_multiplier_id) {  // ADDED ON 2025-04-28 by Lillian Lau.
...
void Restrictions::calculateWeeklyAverageWaterPrices(
    ...
    if (priceMultipliers) {
        ...
        for (unsigned long s = 0; s < priceMultipliers->size(); ++s) { // stages loop
           ...
           for (int w = 0; w < (int) (WEEKS_IN_YEAR + 1); ++w)
                restricted_weekly_average_volumetric_price[s][w] =
                        monthly_average_price[(int) (w / WEEKS_IN_MONTH)]/1.0e6;
        }
    }
}
...
void Restrictions::setRealization(unsigned long realization_id, 
    ...
    // ADDED ON 2025-08-11 by Lillian Lau.
    // Shift the stage_triggers by the action multiplier.
    double action_multiplier = actions_rdm.at((unsigned long) action_multiplier_id);
    for (double& st : stage_triggers) {
        st = min(1.0, max(0., st + action_multiplier));
    }
}
