...
// ADDING PARQUET CAPABILITY ON 2025-05-27 by Lillian Lau.
#include <iostream>
#include <stdexcept>
#include <arrow/api.h>
...
// ADDING NEW FUNCTION TO READ IN PARQUET FILES ON 2025-05-27 by Lillian Lau.
vector<vector<double>> Utils::parse2DParquetFile(string file_name, 
                                                unsigned long n_realizations) {

    // Check file exists
    struct stat buffer;
    if (stat(file_name.c_str(), &buffer) != 0) {
        throw invalid_argument("File \"" + file_name + "\" not found.");
    }
    
    // Represents the Parquet file opened as an Arrow readable file.
    ...
    arrow::Status st1 = parquet::arrow::OpenFile(infile, arrow::default_memory_pool(), &reader);
    if (!st1.ok()) {
        throw runtime_error("Error opening parquet file: " + st1.ToString());
    }

    // Read the entire file into an Arrow Table.
    shared_ptr<arrow::Table> full_table;
    arrow::Status st2 = reader->ReadTable(&full_table);
    if (!st2.ok()) {
        throw runtime_error("Error reading parquet data: ");
    }

    unsigned long num_rows = full_table->num_rows();
    unsigned long num_cols = full_table->num_columns();
    //unsigned long selected_rows = min(num_rows, n_realizations);
    unsigned long selected_rows = num_rows;
    
    shared_ptr<arrow::Table> table = full_table->Slice(0, selected_rows);

    // Pull out rows    
    vector<vector<double>> result;
    result.reserve(selected_rows);

    for (unsigned long row = 0; row < selected_rows; ++row) {
        vector<double> row_data;
        row_data.reserve(num_cols);

        for (unsigned long col = 0; col < num_cols; ++col) {
            auto chunked = table->column(col);
            // assume single-chunk
            auto darr = static_pointer_cast<arrow::DoubleArray>(chunked->chunk(0));
            row_data.push_back(darr->Value(row));
        }

        result.push_back(move(row_data));
    }
