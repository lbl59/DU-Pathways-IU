...
int main(int argc, char *argv[]) {
    ...
    string output_dir = DEFAULT_OUTPUT_DIR;  // ADDED ON 2025-07-16 by Lillian Lau.
    ...
    int c;
    while ((c = getopt(argc, argv, "?s:u:T:r:t:d:f:l:m:v:p:b:i:n:o:e:y:S:A:D:R:U:P:W:I:M:C:O:B:")) != -1) {
        switch (c) {
            case '?':
                fprintf(stdout,
                        "%s\n"
                        ...
                        //"\t-v: verbose mode (false)\n"  // CHANGED ON 2025-07-16 BY LILLIAN LAU 
                        "\t-v: output file name\n"
                        //"\t-c: tabular output (really big files, false)\n"// CHANGED ON 2025-07-16 BY LILLIAN LAU 
                        "\t-c: Parquet output format \n"// CHANGED ON 2025-07-16 BY LILLIAN LAU 
                        "\t-p: plotting output (CSV files)\n"
                        ...
                        "\t-B: number of sets of bootstrap samples for bootstrap analysis.\n"
                        "\t-D: RDM data directory.\n"  // ADDED ON 2025-05-27 by Lillian Lau.
                        "\t-R: RDM sample number\n"
                        "\t-U: Utility RDM file\n"
                        "\t-P: Policies RDM file\n"
                        ...
                        " be added to folder names (e.g., _high or _inflows_bs)\n"
                        "\t-M: Mode (format) of the input ROF and hydroclimatic files "
                        "(0: CSV, 1: Parquet)\n",
            ...
            case 'v':
                ...
                output_dir = optarg;  // ADDED ON 2025-07-16 by Lillian Lau.
                break;
            case 'p':
                plotting = static_cast<bool>(atoi(optarg));
                break;
            case 'c':
                parquet = static_cast<bool>(atoi(optarg));
                break;
            case 'b':
            ...
            case 'y':
                ...
                break;
            case 'D':
                rdm_directory = optarg; // ADDED ON 2025-05-27 by Lillian Lau.
                break;
            case 'R':
                rdm_no = atoi(optarg);
                break;
    ...
    problem.setRDMDirectory(rdm_directory);  // ADDED ON 2025-05-27 by Lillian Lau.
    problem.setOutputDirectory(output_dir);  // ADDED ON 2025-07-16 by Lillian Lau.
    // Set up input/output suffix, if necessary.
    if (strlen(inflows_evap_directory_suffix.c_str()) > 2) {
        problem.set_inflow_evap_demand_suffix(inflows_evap_directory_suffix);
    }
    ...
    if (strlen(utilities_rdm_file.c_str()) > 2) {
        ...
        if (rdm_no != NON_INITIALIZED) {
            ...
            if (strlen(actions_rdm_file.c_str()) <= 2) {
                actions_rdm = std::vector<vector<double>>(n_realizations, vector<double>(11, 0.0));
            } else {
                auto actions_rdm_row = Utils::parse2DCsvFile(system_io + actions_rdm_file)[rdm_no];
                actions_rdm = std::vector<vector<double>>(n_realizations, actions_rdm_row);
            }
            ...
            problem.setRDMReevaluation((unsigned long) rdm_no, utilities_rdm,
                                       water_sources_rdm, policies_rdm, actions_rdm);
            ...
            */
            ...
            if (strlen(actions_rdm_file.c_str()) <= 2) {
                ...
                actions_rdm = Utils::parse2DCsvFile(system_io + actions_rdm_file);
            }
    ...
    problem.setImport_export_rof_tables(import_export_rof_table, (int) n_weeks, mode,
            system_io + rof_tables_directory);
    problem.readInputData(mode);
    ...
    if (!run_optimization) {
        ...
        // Run model
        if (first_solution == -1) {  // if only one solution is to be run
            ofstream objs_file;
            Utils::createDir(system_io + output_dir);
            string file_name = system_io + output_dir + "Objectives" +
                                (rdm_no == NON_INITIALIZED ? "" : "_RDM" + to_string(
                                        rdm_no)) +
                                "_sols" + to_string(standard_solution) +
                                "_to_" + to_string(standard_solution+1) +
                                ".csv";
            objs_file.open(file_name);
            
            cout << endl << endl << endl << "Running solution " << standard_solution << endl;
            ...
            if (n_sets > 0 && n_bs_samples > 0) {
                ...
                problem.printTimeSeriesAndPathways(plotting, parquet);
                objectives = problem_ptr->calculateAndPrintObjectives(true);  // UPDATED ON 2025-08-25 by Lillian Lau.
            }

            problem_ptr->destroyDataCollector();
            
            string buffer;
            int num_objectives = size(objectives);
            for (int j = 0; j < num_objectives; ++j) {
                    buffer += std::to_string(objectives[j]);
                    if (j < num_objectives - 1) {
                            buffer += ",";
                    }
            }
            ...
            ofstream objs_file;
            string file_name = output_dir + BAR + "Objectives" + 
                                (rdm_no == -1 ? "" : "_RDM" + to_string(rdm_no)) +
                               "_sols" + to_string(first_solution) + 
                               "_to_" + to_string(last_solution) + ".csv";
            objs_file.open(file_name);
            printf("Objectives file will be printed at %s.\n", file_name.c_str());
            
            for (int s = first_solution; s < last_solution; ++s) {
                cout << endl << endl << endl << "Running solution " << s << endl;
        ...
        if (rank == 0) {
            ...
            cout << "Total number of processes launched: " << world_size << endl;
        }
        ...
        char output_file_name[256], runtime_file_name[256];	    
        FILE* outputFile = nullptr;

        // ADDED BY LILLIAN LAU ON 06-28-2025: Convert output_dir to char array
        sprintf(io_directory, "%s", output_dir.c_str());
        Utils::createDir(string(output_dir));

	    //sprintf(output_directory, "%s%s", system_io.c_str(), DEFAULT_OUTPUT_DIR.c_str());
	    //Utils::createDir(string(output_directory));

        sprintf(output_file_name, "%srefset_M%lu_NFE%lu_bs200_seed%lu.set", io_directory, n_islands, nfe, seed);
        sprintf(runtime_file_name, "%sruntime_MM%lu_M%lu_NFE%lu_bs200_seed%lu.runtime", io_directory, rank, n_islands, nfe, seed); // runtime

        BORG_Algorithm_output_runtime(runtime_file_name);
