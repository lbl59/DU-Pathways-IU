...
// ADDED PARQUET HEADERS ON 2025-05-30 by Lillian Lau.
#include <arrow/api.h>
...
ContinuityModelROF::ContinuityModelROF(vector<WaterSource *> water_sources, const Graph &water_sources_graph,
                                       ...
                                       vector<double> &actions_rdm, // ADDED ON 2025-04-28 by Lillian Lau.
        ...
        : ContinuityModel(water_sources, utilities, min_env_flow_controls,
                          ...
                          water_sources_rdm, actions_rdm, // ADDED ON 2025-04-28 by Lillian Lau.
                          realization_id),
...
void ContinuityModelROF::printROFTableParquet(const string &folder) {
    auto pool = arrow::default_memory_pool();

    // for each utility, create a parquet file with the ROF table.
    for (int u = 0; u < n_utilities; ++u) {
        int num_weeks = (int) ut_storage_to_rof_table[u].get_i();
        int num_columns = NO_OF_INSURANCE_STORAGE_TIERS;
        ...
        // Build the column-oriented arrays first
        for (int t = 0; t < NO_OF_INSURANCE_STORAGE_TIERS; ++t) {
            vector<double> row_data;
            row_data.reserve(num_weeks);
            
            for (int64_t w = 0; w < num_weeks; ++w) {
                auto data = ut_storage_to_rof_table[u].getPointerToElement(w, 0);
                //builders[t]->Append(row_ptr[t]);
                row_data.push_back(data[t]);
            }
            // Append the data to the builder for this tier
            PARQUET_THROW_NOT_OK(builders[t]->AppendValues(row_data));
            shared_ptr<arrow::Array> array;
            PARQUET_THROW_NOT_OK(builders[t]->Finish(&array));
            columns.push_back(array);
            fields.push_back(arrow::field("tier_" + to_string(t), arrow::float64()));
            }

        // Create the table 
        auto schema = make_shared<arrow::Schema>(fields);
        auto table = arrow::Table::Make(schema, move(columns));

        string file_name = folder + "tables_r" + to_string(realization_id) + \
            "_u" + to_string(u) + ".parquet";

        shared_ptr<arrow::io::FileOutputStream> outputFile;
        PARQUET_ASSIGN_OR_THROW(
            outputFile, arrow::io::FileOutputStream::Open(file_name));
        ...
        PARQUET_THROW_NOT_OK(
            parquet::arrow::WriteTable(*table, pool, outputFile, 
                num_weeks, writer_properties));
    }
}
