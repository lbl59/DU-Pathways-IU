...
def plot_kde(df, dv, title, ax, color, setup,
             ...
             linestyle='--'):
    data_dv = (df[dv] - df[dv].min()) / (df[dv].max() - df[dv].min())
    #data_dv = df[dv]
    kde = sm.nonparametric.KDEUnivariate(data_dv)
    kde.fit(bw=0.06)
    ...
    for yy in y:
        ...
        if np.isnan(xx): 
            ...
            x.append(xx[0])
    x = np.array(x)
    ax.plot(y, x, color=color, linewidth=2, label=setup)  # KDE line
    ax.fill_between(y, x, lw=1, color=color, alpha=0.05)
...
def plot_polar_families(decision_vars_all, dvs_selected, dvs_color_dict, dv_names, utility, ax):
    ...
    # plot each decision variable on the polar plot 
    num_vars = dv_toplot_arr.shape[1]
    theta = np.linspace(0,2*np.pi, num_vars)
    r = np.ones(num_vars)  # radius at edge
    ...
    for i in range(0, dv_toplot_arr.shape[0]):
        ax.plot(theta, dv_toplot_arr[i,:], color='lightgrey', alpha=0.8)
        # add a line at the middle of the plot
        ax.plot(theta, np.array([0.5]*len(theta)), linestyle='--', linewidth=0.8, zorder=500)
        if i == 0:
            ax.plot(theta, dv_toplot_arr[i,:], color='lightgrey', 
                    alpha=0.8, label='all_dvs')
    '''
    ...
    for i in range(0, dv_toplot_highcoop.shape[0]):
        ...
        if i == 0:
            ax.plot(theta, dv_toplot_highcoop[i,:], color=dvs_color_dict['high_coop'], 
                    alpha=0.5, label='High Co-op', linewidth=1.0)
            ax.fill_between(theta, dv_toplot_highcoop[i,:], lw=1, color=dvs_color_dict['high_coop'], alpha=0.1)
    ax.plot(np.append(x, x[0]), np.append(y, y[0]), color='darkgrey', linewidth=1, 
            transform=ax.transData._b, zorder=1000)
    
    radii = np.linspace(0.2, 1.0, 5)  # grid radii

    for r in radii:
        x = r * np.cos(theta)
        y = r * np.sin(theta)
        # Close the polygon by repeating the first point
        ax.plot(np.append(x, x[0]), np.append(y, y[0]), color='gainsboro', linestyle='--', linewidth=0.7, 
